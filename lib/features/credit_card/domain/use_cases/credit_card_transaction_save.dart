...
class CreditCardTransactionSave implements ICreditCardTransactionSave {
  ...
  Future<CreditCardTransactionEntity> save(CreditCardTransactionEntity entity) async {
    ...
    final dates = _statementDates.generateDates(closingDay: creditCard.invoiceClosingDay, dueDay: creditCard.invoiceDueDay, baseDate: entity.date);

    //Se não existir nenhuma fatura superior a data da transação ou a fatura encontrada for de um mês posterior
    if (statement == null || statement.invoiceClosingDate.year != dates.closingDate.year || statement.invoiceClosingDate.month != dates.closingDate.month) {
      //Monta uma nova fatura
      statement = CreditCardStatementEntity(
        id: null,
        createdAt: null,
        deletedAt: null,
        invoiceClosingDate: dates.closingDate,
        invoiceDueDate: dates.dueDate,
